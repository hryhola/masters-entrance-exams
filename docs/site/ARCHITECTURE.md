...
Реалізований модуль `features/practice` складається з:

- чистої моделі `PracticeSession` і reducer переходів;
- детермінованого добору унікальних питань для повного, тематичного й
  швидкого режимів;
- `PracticeSessionProvider`, який тримає активні сесії в пам'яті вкладки;
- окремого UI налаштування, проходження питання та результату.

Timer зберігає абсолютний `deadlineAt`, тому після throttling фонової
вкладки залишок перераховується за поточним часом, а не за кількістю
виконаних interval callback. Параметри timer і доступні розміри швидкої
сесії задаються через `ExamDefinition`.

Постійне збереження не змішується з reducer. На Етапі 4 context
використовуватиме `StorageRepository`, не змінюючи доменні переходи або
компоненти питання.
